...
RUN git clone -b use-hphpc --depth=1 git://github.com/facebook/hhvm.git

ENV CMAKE_PREFIX_PATH=/hphpc HPHP_HOME=/hphpc/hhvm HPHP_LIB=/hphpc/hhvm/bin USE_HHVM=1

RUN git clone -b release-1.4.14b-stable --depth=1 git://github.com/libevent/libevent.git
RUN cd libevent && \
...
RUN git clone -b curl-7_28_1 --depth=1 git@github.com:curl/curl.git
RUN cd curl && \
    ./buildconf && \
    ./configure -prefix=$CMAKE_PREFIX_PATH && \
    make && \
...
RUN git clone --depth=1 git@github.com:google/glog.git
RUN cd glog && \
    ./configure --prefix=$CMAKE_PREFIX_PATH && \
    make && \
    make install
